...
def make_optimizer_and_scheduler(cfg, policy):
    if cfg.policy.name == "act":
        ...
        optimizer_params_dicts_vae = [
            {
                "params": [
                    p
                    for n, p in policy.named_parameters()
                    if (not n.startswith("model.discriminator")) and p.requires_grad
                ]
            },
        ]
        ...
        optimizer_params_dicts_dis = [
            {
                "params": [
                    p
                    for n, p in policy.named_parameters()
                    if n.startswith("model.discriminator") and p.requires_grad
                ]
            },
        ]
        ...
        optimizer = [
            optimizer_vae,
            optimizer_dis,
            optimizer_params_dicts_vae[0]["params"],
            optimizer_params_dicts_dis[0]["params"],
        ]
        lr_scheduler = None
    elif cfg.policy.name == "_act":
...
def update_policy(
    ...
    if isinstance(optimizer, Optimizer):
        ...
        # optimizer is a list of optimizers
        start_time = time.perf_counter()
        device = get_device_from_parameters(policy)
        policy.train()
        ...
        L_prior = output_dict["L_prior"]
        L_like = output_dict["L_like"]
        ...
        # 收集日志信息
        info = {
            # "loss": loss_enc + loss_dec + loss_dis,
            "loss": loss_vae + loss_dis,
            "vae_loss": loss_vae,
            # "enc_loss": loss_enc,
            # "dec_loss": loss_dec,
            # "grad_norm": float((grad_norm_enc + grad_norm_dec + grad_norm_dis)/3),
            "grad_norm": float((grad_norm_vae + grad_norm_dis) / 2),
            # "grad_norm_enc": float(grad_norm_enc),
            # "grad_norm_dec": float(grad_norm_dec),
            "grad_norm_vae": float(grad_norm_vae),
            "grad_norm_dis": float(grad_norm_dis),
            # "lr": np.mean([optimizer_enc.param_groups[0]["lr"], optimizer_dec.param_groups[0]["lr"], optimizer_dis.param_groups[0]["lr"]]),
            "lr": 2e-6,
            # "lr_enc": optimizer_enc.param_groups[0]["lr"],
            # "lr_dec": optimizer_dec.param_groups[0]["lr"],
            # "lr_dec": optimizer_vae.param_groups[0]["lr"],
            # "lr_dis": optimizer_dis.param_groups[0]["lr"],
            "update_s": time.perf_counter() - start_time,
            **{
                k: v
                for k, v in output_dict.items()
                if k not in ["L_prior", "L_like", "L_gan", "L_l1", "L_dis"]
            },
        }
...
def train(cfg: DictConfig, out_dir: str | None = None, job_name: str | None = None):
    ...
    policy.train()
    offline_step = 0
    for s in range(step, cfg.training.offline_steps):
        if offline_step == 0:
